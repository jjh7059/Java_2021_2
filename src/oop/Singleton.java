...
public class Singleton {
	...
	//시스템 필드(_변수명) : 클래스 내부에서만 사용될 값을 저장하기 위한 필드 
	...
	//정적영역(Static Block) : 클래스가 메모리(Static Memory)에 저장된 후 자동 실행될 명령을
	//작성하는 영역 - 프로그램에서 한번만 실행
	// => static 필드 또는 static 메소드만 사용 가능
	//클래스로 인스턴스를 생성하여 시스템 필드(Static Field)에 저장
	static {
		_instance = new Singleton();
	}
	
	//시스템 필드(Static Field)에 저장된 인스턴스를 반환하는 정적 메소드 선언
	// => 메소드를 호출하여 시스템 필드에 저장된 인스턴스를 제공 받아 사용 가능
